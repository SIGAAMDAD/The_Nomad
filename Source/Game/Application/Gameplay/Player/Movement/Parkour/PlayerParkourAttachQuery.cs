...
namespace Nomad.Game.Application.Gameplay.Player.Movement.Parkour
{
	internal sealed class PlayerParkourAttachQuery
	{
		...
		public PlayerParkourAttachQuery(
			...
			TraversalQueryBuffer queryBuffer,
		...
		)
		{
			...
			Vector3 normalizedFacing = PlayerParkourMath.SafeNormalized( facing, Vector3.Forward );
			...
			for ( int graphIndex = 0; graphIndex < graphCount; graphIndex++ ) {
				...
				for ( int i = 0; i < _queryBuffer.Count; i++ ) {
					...
					float facingAnchorDot = normalizedFacing.Dot( toAnchorDir );
					if ( facingAnchorDot < _settings.MinFacingAnchorDot ) {
						continue;
					}

					Vector3 anchorNormal = PlayerParkourMath.AnchorNormal( anchor );
					float facingWallDot = normalizedFacing.Dot( -anchorNormal );
