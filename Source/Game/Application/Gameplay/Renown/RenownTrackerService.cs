...
namespace Nomad.Game.Application.Gameplay.World.Renown
{
	...
	*/
	...
	internal sealed class RenownTrackerService : IRenownTrackerService
	{
		...
		private readonly IGameEvent<RenownScoreChangedEventArgs> _renownScoreChanged = default;

		public IGameEvent<RenownTierChangedEventArgs> RenownTierChanged => _renownTierChanged;
		private readonly IGameEvent<RenownTierChangedEventArgs> _renownTierChanged = default;

		private readonly ConcurrentDictionary<InternString, RenownStatus> _regionRenown = new();
		private readonly ImmutableArray<RenownTierDefinition> _tiers;
		...
		*/
		...
		public void ApplyDecay( InternString regionId, float amount, int worldDay )
		{
			if ( amount <= 0.0f ) {
				return;
			}
		...
		*/
		...
		/// <param name="regionId"></param>
		/// <returns></returns>
		public float GetRenownRatio( InternString regionId )
		{
			...
			if ( nextTier != null ) {
				return 1.0f; // already at highest tier
			}
		...
		*/
		...
		/// <param name="regionId"></param>
		/// <returns></returns>
		public RenownStatus GetStatus( InternString regionId )
		{
		...
		*/
		...
		/// <param name="regionId"></param>
		/// <returns></returns>
		public RenownTier GetTier( InternString regionId )
		{
		...
		*/
		...
		public void SetHistoricalFloor( InternString regionId, InternString sourceId, float floorValue )
		{
			var status = GetOrCreateStatus( regionId );
		...
		*/
		...
		/// <param name="regionId"></param>
		/// <returns></returns>
		private RenownStatus GetOrCreateStatus( InternString regionId )
		{
			return _regionRenown.GetOrAdd( regionId, f => new RenownStatus() );
		}
	};
};
