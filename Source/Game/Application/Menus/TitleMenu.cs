...
//			SetMenu( ref CreditsMenu, MenuState.Credits, "res://scenes/menus/credits_menu.tscn" );
		...
		*/
		private void OnKonamiCodeActivated() {
			Logger?.PrintLine( "========== Meme Mode Activated ==========" );
...
//			UIAudioManager.PlayTheme();
			...
			AddChild( SceneCache.Instance.GetCached( "res://scenes/menus/menu_background.tscn" ).Instantiate<Control>() );

			var eventBus = ServiceRegistry.Get<IGameEventBusService>();
			eventBus.ConnectSignal( GetNode( "KonamiCode" ), "success", this, OnKonamiCodeActivated );
