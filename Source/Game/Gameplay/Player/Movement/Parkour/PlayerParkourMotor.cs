...
namespace Nomad.Game.Gameplay.Player.Movement.Parkour
{
	internal sealed class PlayerParkourMotor
	{
		...
		{
			float alpha = Math.Clamp( _settings.AnchorSnapSpeed * dt, 0.0f, 1.0f );
			_prefab.GlobalPosition = _prefab.GlobalPosition.Lerp( PlayerParkourMath.AnchorPosition( anchor ).ToGodot(), alpha );
		}

		public void AttachImmediate( in TraversalAnchor anchor )
		{
			_prefab.GlobalPosition = PlayerParkourMath.AnchorPosition( anchor ).ToGodot();
		...
		{
			Godot.Vector3 forward = -wallNormal.ToGodot();
			forward.Y = 0.0f;
			...
			Godot.Vector3 currentForward = -_prefab.GlobalTransform.Basis.Z;
			...
			float alpha = Math.Clamp( _settings.TurnToWallSpeed * dt, 0.0f, 1.0f );
			Godot.Vector3 blended = currentForward.Slerp( forward, alpha ).Normalized();

			_prefab.LookAt( _prefab.GlobalPosition + blended, Godot.Vector3.Up );
		}
		...
		public void FaceWallImmediate( Vector3 wallNormal )
		{
			...
			_prefab.LookAt( _prefab.GlobalPosition + forward.Normalized(), Godot.Vector3.Up );
		}

		public void TraverseEdge( PlayerParkourRuntime runtime, float dt )
		{
			float t = Math.Clamp( runtime.EdgeElapsed / MathF.Max( runtime.EdgeDuration, _settings.MinimumEdgeDuration ), 0.0f, 1.0f );
			float eased = PlayerParkourMath.SmoothStep( t );

			Godot.Vector3 targetPosition = runtime.EdgeStart.Lerp( runtime.EdgeEnd, eased );
			Vector3 targetNormal = PlayerParkourMath.SafeNormalized(
				runtime.EdgeStartNormal.Lerp( runtime.EdgeEndNormal, eased ).ToSystem(),
				runtime.EdgeStartNormal.ToSystem()
			);

			Godot.Vector3 delta = targetPosition - _prefab.GlobalPosition;
		...
		{
			_prefab.Velocity = (PlayerParkourMath.AnchorNormal( anchor ) * _settings.DropPushOffSpeed + -Vector3.UnitY * _settings.DropDownSpeed).ToGodot();
		}
	};
};
