...
namespace Nomad.Game.Gameplay.Player.Movement
{
	internal sealed class PlayerParkourController : IPlayerParkourController
	{
		...
		)
		{
			...
			_animation = animationState ?? new PlayerParkourAnimationState( prefab.GetTree(), prefab.GetNodeOrNull<Godot.AnimationTree>( "AnimationTree" ) );

			_settings = new PlayerParkourSettings();
			_runtime = new PlayerParkourRuntime();
			_input = new PlayerParkourInputAdapter();
			_queryBuffer = new PooledList<int>( Math.Max( _settings.InitialQueryBufferCapacity, 8 ) );
		...
		public void HandlePlayerCameraStatusChanged( in PlayerCameraStatusChangedEventArgs args )
		{
			...
			_hasCamera = true;
			_cameraForward = PlayerParkourMath.SafeNormalized( args.Forward, -Vector3.UnitZ );
		}
		...
		private void PhysicsTraversingEdge( float dt )
		{
			...
			float t = Math.Clamp( _runtime.EdgeElapsed / MathF.Max( _runtime.EdgeDuration, _settings.MinimumEdgeDuration ), 0.0f, 1.0f );
		...
		private bool TryAttachToBestAnchor()
		{
			...
			Vector3 bodyPosition = _prefab.GlobalPosition.ToSystem();
		...
		private void StartEdge( TraversalGraph graph, int edgeIndex )
		{
			...
			_runtime.EdgeEnd = _edgeMotion.GetTargetPosition( from, to, edge ).ToGodot();
			_runtime.EdgeStartNormal = PlayerParkourMath.AnchorNormal( from ).ToGodot();
			_runtime.EdgeEndNormal = PlayerParkourMath.AnchorNormal( to ).ToGodot();
			_runtime.EdgeElapsed = 0.0f;
		...
		{
			var basisZ = -_prefab.GlobalTransform.Basis.Z.ToSystem();

			Vector3 forward = _hasCamera ? _cameraForward : basisZ;
			forward.Y = 0.0f;
			return PlayerParkourMath.SafeNormalized( forward, basisZ );
		}
