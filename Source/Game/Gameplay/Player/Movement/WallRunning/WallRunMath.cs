...
namespace Nomad.Game.Gameplay.Player.Movement.WallRunning
{
	...
	*/
	...
	internal static class WallRunMath
	{
		...
		public const float TINY_LENGTH = 1E-06f;
		...
		{
			float normalSpeed = Vector3.Dot( value, normal );
		...
		public static Vector3 SafePlanarNormalized( Vector3 value, Vector3 fallback )
		{
			value.Y = 0.0f;
			fallback.Y = 0.0f;
			return SafeNormalized( value, SafeNormalized( fallback, -Vector3.UnitZ ) );
		}
		...
		public static Vector3 PlanarRight( Vector3 forward )
		{
			...
			return SafePlanarNormalized( right, Vector3.UnitX );
		}
	};
};
