...
namespace Nomad.Game.Gameplay.Renown.Regional
{
	...
	*/
	...
	internal sealed class RenownDecayService : IDisposable
	{
		...
		public RenownDecayService( RenownTrackerService tracker, ICalendarService calendar, ICVarSystemService cvarSystem )
		{
			...
			_gracePeriodDays = new CVarBinding<int>( cvarSystem.GetCVarOrThrow<int>( RenownCVarRegistry.DecayGracePeriod.Name ) );
			_baseDailyDecay = new CVarBinding<float>( cvarSystem.GetCVarOrThrow<float>( RenownCVarRegistry.BaseDailyDecay.Name ) );
		}
		...
		public void Dispose()
		{
			...
			_gracePeriodDays.Dispose();
			_baseDailyDecay.Dispose();
