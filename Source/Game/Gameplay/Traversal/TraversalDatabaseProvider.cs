...
namespace Nomad.Game.Gameplay.Traversal
{
	...
	{
		private const int FALLBACK_SOURCE_ID = 0;

		private readonly DenseIdMap<TraversalGraphSource> _activeSources = new DenseIdMap<TraversalGraphSource>( 16 );
		...
		private int _nextSourceId = FALLBACK_SOURCE_ID + 1;
		...
		{
			if ( registrationId < FALLBACK_SOURCE_ID + 1 ) {
				return;
			}
