    ...
    public class Action {
		...
		internal bool IsPossible(Dictionary<string, object?> state) {
			foreach (var kvp in preconditions) {
				...
				object value = state[ kvp.Key ];
				if ( value == null && value != kvp.Value ) return false;
				else if ( value == null && value == kvp.Value ) continue;
				if ( value is object obj && !obj.Equals( kvp.Value ) ) return false;
			}
			foreach (var kvp in comparativePreconditions) {
				...
				if ( value is object obj && kvp.Value.Value is object obj2 ) {
					switch ( kvp.Value.Operator ) {
					case ComparisonOperator.LessThan:
						return Utils.IsLowerThan(obj, obj2);
					case ComparisonOperator.GreaterThan:
						return Utils.IsHigherThan(obj, obj2);
					case ComparisonOperator.LessThanOrEquals:
						return Utils.IsLowerThanOrEquals(obj, obj2);
					case ComparisonOperator.GreaterThanOrEquals:
						return Utils.IsHigherThanOrEquals(obj, obj2);
					};
				}
				else return false;
			}
		...
		internal List<Dictionary<string, object?>> GetPermutations(Dictionary<string, object?> state) {
			...
			Span<int> indices = stackalloc int[ permutationParameters.Count ];
			Span<int> counts = stackalloc int[ permutationParameters.Count ];
			for ( int i = 0; i < permutationParameters.Count; i++ ) {
				string parameter = permutationParameters[i];
				indices[i] = 0;
				if (outputs[parameter].Count == 0) return combinedOutputs;
				counts[i] = outputs[parameter].Count;
			}
			while (true) {
		...
		internal void ApplyEffects(Dictionary<string, object?> state) {
			...
			foreach (var kvp in arithmeticPostconditions) {
				...
				object value = state[ kvp.Key ];
				if ( value is int stateInt && kvp.Value is int conditionInt ) state[kvp.Key] = stateInt + conditionInt;
				else if ( value is float stateFloat && kvp.Value is float conditionFloat ) state[kvp.Key] = stateFloat + conditionFloat;
				else if ( value is double stateDouble && kvp.Value is double conditionDouble ) state[kvp.Key] = stateDouble + conditionDouble;
				else if ( value is long stateLong && kvp.Value is long conditionLong ) state[kvp.Key] = stateLong + conditionLong;
				else if ( value is decimal stateDecimal && kvp.Value is decimal conditionDecimal ) state[kvp.Key] = stateDecimal + conditionDecimal;
				else if ( value is DateTime stateDateTime && kvp.Value is TimeSpan conditionTimeSpan ) state[kvp.Key] = stateDateTime + conditionTimeSpan;
			}
		...
		private static bool IndicesAtMaximum(Span<int> indices, Span<int> counts) {
			for (int i = 0; i < indices.Length; i++) if (indices[i] < counts[i] - 1) return false;
			return true;
		}

		private static void IncrementIndices(Span<int> indices, Span<int> counts) {
