...
namespace MountainGoap {
	...
	public class Agent {
		...
		public Agent(string? name = null, Dictionary<string, object?>? state = null, Dictionary<string, object?>? memory = null, BaseGoal[]? goals = null, Action[]? actions = null, Sensor[]? sensors = null, float costMaximum = float.MaxValue, int stepMaximum = int.MaxValue) {
			...
			PlanThread = new Thread(new ThreadStart(() => { Planner.Plan(this, CostMaximum, StepMaximum); }));
		}
		...
		private Thread PlanThread = null;
		...
		private void Execute() {
			if (CurrentActionSequences.Count > 0) {
				for ( int i = 0; i < CurrentActionSequences.Count; i++ ) {
					List<Action> sequence = CurrentActionSequences[i];
					if (sequence.Count > 0) {
