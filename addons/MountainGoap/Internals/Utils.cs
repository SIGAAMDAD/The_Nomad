...
#pragma warning disable S3267 // Loops should be simplified with "LINQ" expressions
				foreach (var kvp in normalGoal.DesiredState) {
					if ( !actionNode.State.ContainsKey(kvp.Key) ) {
						return false;
					}
					object value = actionNode.State[ kvp.Key ];
					if ( value == null && value != normalGoal.DesiredState[kvp.Key]) return false;
					else if ( value is object obj && obj != null && !obj.Equals(normalGoal.DesiredState[kvp.Key])) return false;
				}
#pragma warning restore S3267 // Loops should be simplified with "LINQ" expressions
				...
				foreach (var kvp in extremeGoal.DesiredState) {
					if (!actionNode.State.ContainsKey(kvp.Key) || !current.State.ContainsKey(kvp.Key)) {
						return false;
					}

					object value1 = actionNode.State[ kvp.Key ];
					object value2 = current.State[ kvp.Key ];
					if (kvp.Value && value1 is object a && value2 is object b && IsLowerThanOrEquals(a, b)) return false;
					else if (!kvp.Value && value1 is object a2 && value2 is object b2 && IsHigherThanOrEquals(a2, b2)) return false;
				}
			}
				...
				foreach (var kvp in comparativeGoal.DesiredState) {
					...
					object value1 = actionNode.State[ kvp.Key ];
					object value2 = comparativeGoal.DesiredState[ kvp.Key ].Value;
					...
					case ComparisonOperator.Equals:
						return value1 is object obj && !obj.Equals(value2) ? false : true;
					case ComparisonOperator.LessThan:
						return value1 is object a && value2 is object b && !IsLowerThan(a, b) ? false : true;
					case ComparisonOperator.GreaterThan:
						return value1 is object a2 && value2 is object b2 && !IsHigherThan(a2, b2) ? false : true;
					case ComparisonOperator.LessThanOrEquals:
						return value1 is object a3 && value2 is object b3 && !IsLowerThanOrEquals(a3, b3) ? false : true;
					case ComparisonOperator.GreaterThanOrEquals:
						return value1 is object a4 && value2 is object b4 && !IsHigherThanOrEquals(a4, b4) ? false : true;
					};
				}
			}
			return true;
