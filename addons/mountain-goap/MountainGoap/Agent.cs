...
namespace MountainGoap {
	...
	public class Agent {
		...
		public void Step( StepMode mode = StepMode.Default ) {
			...
			switch ( mode ) {
			case StepMode.Default:
				StepAsync();
				break;
			case StepMode.OneAction:
				if ( !IsBusy ) {
					Planner.Plan( this, CostMaximum, StepMaximum );
				}
				Execute();
				break;
			case StepMode.AllActions:
				if ( !IsBusy ) {
					Planner.Plan( this, CostMaximum, StepMaximum );
				}
				while ( IsBusy ) {
		...
		private void Execute() {
			if ( CurrentActionSequences.Count > 0 ) {
				...
				for ( int s = 0; s < CurrentActionSequences.Count; s++ ) {
					if ( CurrentActionSequences[ s ].Count > 0 ) {
						ExecutionStatus status = CurrentActionSequences[ s ][ 0 ].Execute( this );
						if ( status != ExecutionStatus.Executing ) {
							CurrentActionSequences[ s ].RemoveAt( 0 );
						}
					} else {
						cullableSequences.Add( CurrentActionSequences[ s ] );
					}
				}
				for ( int s = 0; s < cullableSequences.Count; s++ ) {
					CurrentActionSequences.Remove( cullableSequences[ s ] );
