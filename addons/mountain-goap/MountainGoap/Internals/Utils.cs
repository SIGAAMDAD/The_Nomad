		...
		internal static bool MeetsGoal( BaseGoal goal, ActionNode actionNode, ActionNode current ) {
			if ( goal is Goal normalGoal ) {
				foreach ( var kvp in normalGoal.DesiredState ) {
					if ( !actionNode.State.TryGetValue( kvp.Key, out object value ) ) {
						return false;
					} else if ( value == null && value != kvp.Value ) {
						return false;
					} else if ( value != null && !value.Equals( kvp.Value ) ) {
						return false;
					}
				}
				...
				foreach ( var kvp in extremeGoal.DesiredState ) {
					if ( !actionNode.State.TryGetValue( kvp.Key, out object value ) ) {
						return false;
					} else if ( !current.State.TryGetValue( kvp.Key, out object currentValue ) ) {
						return false;
					} else if ( kvp.Value && IsLowerThanOrEquals( value, currentValue ) ) {
						return false;
					} else if ( !kvp.Value && IsHigherThanOrEquals( value, currentValue ) ) {
						return false;
					}
				}
				...
				foreach ( var kvp in comparativeGoal.DesiredState ) {
					if ( !actionNode.State.TryGetValue( kvp.Key, out object value ) ) {
						...
						return kvp.Value.Operator switch {
							ComparisonOperator.Equals => value.Equals( kvp.Value ),
							ComparisonOperator.LessThan => IsLowerThan( value, kvp.Value ),
							ComparisonOperator.GreaterThan => IsHigherThan( value, kvp.Value ),
							ComparisonOperator.LessThanOrEquals => IsLowerThanOrEquals( value, kvp.Value ),
							ComparisonOperator.GreaterThanOrEquals => IsHigherThanOrEquals( value, kvp.Value ),
							_ => false,
						};
					}
				}
