...
func set_quests(quests: Array[Resource]) -> void:
	var quest_list:Array[QuestResource]
	
	quest_list.resize( quests.size() )
	for quest in quests:
		quest_list.push_back( quest as QuestResource )
	
	clear()
	_quests.assign( quest_list )
