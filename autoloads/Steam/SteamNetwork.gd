...
signal player_joined_lobby( steamId: int )
signal player_left_lobby( steamId: int )
...
func _ready():
	...
	SteamLobby.client_joined_lobby.connect(_init_p2p_session)
	SteamLobby.client_left_lobby.connect(_close_p2p_session)
...
func _init_p2p_session(steam_id):
	...
	emit_signal( "player_joined_lobby", steam_id )
...
func _close_p2p_session(steam_id):
	...
	emit_signal( "player_left_lobby", steam_id )
...
func _handle_packet(sender_id, payload: PackedByteArray):
	if payload.size() == 0:
		...
		return
	print( "got p2p packet from %s" % sender_id )
