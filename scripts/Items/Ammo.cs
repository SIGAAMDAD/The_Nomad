...
public readonly struct Ammo {
	...
	*/
	...
	public Ammo( Resource? resource ) {
		...
		LoadProperty<Curve>( properties, "damage_falloff", out DamageFalloff );
		LoadProperty<AudioStream>( properties, "pickup_sfx", out PickupSfx );
		LoadProperty<float>( properties, "damage", out Damage );
		LoadProperty<float>( properties, "range", out Range );
		LoadProperty<float>( properties, "velocity", out Velocity );
		LoadProperty<AmmoType>( properties, "type", out Type );

		if ( Type == AmmoType.Pellets ) {
			LoadProperty<ShotgunBullshit>( properties, "shotgun_bullshit", out ShotgunBullshit );
			LoadProperty<int>( properties, "pellet_count", out PelletCount );
		}
	}
