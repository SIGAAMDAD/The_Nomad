  ...
  # more actions
	...
	for action in _actions:
		if not action.is_valid():
			print( "action isn't valid ", action.get_action_name() )
			continue
	...
	# desired state
		for s in desired_state:
			if desired_state[s] == effects.get(s):
				...
				print( "adding action ", action.get_action_name(), " to plan" )
...
func _transform_tree_into_array(p, blackboard):
	...
	if p.children.size() == 0:
		plans.push_back( { "actions": [p.action], "cost": p.action.get_cost( blackboard ) if p.action is GoapAction else 0 } )
		return plans
